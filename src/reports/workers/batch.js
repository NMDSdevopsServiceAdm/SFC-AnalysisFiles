...
const findWorkersByBatch = (batchNum) =>
  db
    ...
    SELECT 'M' || DATE_PART('year',(b."RunDate" - INTERVAL '1 day')) || LPAD(DATE_PART('month',(b."RunDate" - INTERVAL '1 day'))::TEXT,2,'0') period,
       ...
       e."NmdsID" nmdsid,
       w."ID" workerid,
       UPPER(MD5(REPLACE(pgp_pub_decrypt(w."NationalInsuranceNumberEncryptedValue" :: bytea, dearmor(convert_from(decode(:private,'base64'),'UTF8')), :passphrase) ,' ','') || TO_CHAR(pgp_pub_decrypt(w."DateOfBirthEncryptedValue" :: bytea, dearmor(convert_from(decode(:private,'base64'),'UTF8')), :passphrase)::date, 'YYYYMMDD'))) wrkglbid,
       1 wkplacestat,
       ...
       EXTRACT(YEAR FROM AGE(pgp_pub_decrypt(w."DateOfBirthEncryptedValue" :: bytea, dearmor(convert_from(decode(:private,'base64'),'UTF8')), :passphrase)::date)) age,
       ...
       CASE "SocialCareStartDateValue" WHEN 'Yes' THEN  "SocialCareStartDateYear" - extract( year FROM pgp_pub_decrypt(w."DateOfBirthEncryptedValue" :: bytea, dearmor(convert_from(decode(:private,'base64'),'UTF8')), :passphrase))  WHEN 'No' THEN -2 ELSE  -1 END startage,
